...
public class Matchmaking : MonoBehaviour {
    ...
    public UnityTransport _transport{get; private set;}
    public const string JoinCodeKey = "j";
    private string _playerId;
    public GameObject lobby_name_input;
    ...
    private void Update(){
        if(!is_connected){
            lobby_name = lobby_name_input.GetComponent<TextMeshProUGUI>().text;
        }
    }
    ...
    private async Task<Lobby> CreateLobby(int maxPlayers, string lobby_name) {
        try {
            // Create a relay allocation and generate a join code to share with the lobby
            var a = await RelayService.Instance.CreateAllocationAsync(maxPlayers);
            var joinCode = await RelayService.Instance.GetJoinCodeAsync(a.AllocationId);
