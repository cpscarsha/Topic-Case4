...
public class PepectMap : MonoBehaviour
{
    ...
    void Update()
    {
        if(g_camera.transform.position.x > g_map[g_middle_map].transform.position.x+g_map_width/2*g_map_scale){
        ...
        else if(g_camera.transform.position.x < g_map[g_middle_map].transform.position.x-g_map_width/2*g_map_scale){
