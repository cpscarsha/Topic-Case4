    ...
    public void Active(){
        ...
        for(int i = 0; i < transform.childCount; i++){
            if((i < g_summon_rate.Length?g_summon_rate[i]:100) > Random.Range(0f, 99.99f)){    
                ...
                int repect_count = 0;
                do{
                    float x = 0.125f+(int)((summon_range.x-0.125f)/0.25f)*0.25f + Random.Range(0, (int)Mathf.Ceil(summon_range.z))*0.25f;
                    float y = summon_range.y + Random.Range(0, (int)Mathf.Ceil(summon_range.w))*0.25f;
